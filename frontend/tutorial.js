window.onload = () => {
    ...
    const Rx = rxjs;
    const {
        Observable, Subject, ReplaySubject, BehaviorSubject
    } = rxjs;
    const {
        buffer,
        bufferCount,
    ...
    function btnValues() {
        Rx.of(11, 22, 33).subscribe(RxJsVisualizer.observerForLine(1))
    }

    function btnRange() {

        Rx.range(5, 3).subscribe(RxJsVisualizer.observerForLine(1))
    }
    ...
    function btnHttp() {
        const users = fetch('https://jsonplacehlder.typicode.com/users/1').then(x => x.json());
    ...
    function btnEvent() {
        ...
        Rx.fromEvent(btn, 'click').subscribe(RxJsVisualizer.observerForLine(0))
    }
    ...
    function btnFilter() {
        ...
        obs.pipe(filter(x => x % 2 === 0)).subscribe(RxJsVisualizer.observerForLine(0, 'even', true))
    }

    function btnMap() {
        RxJsVisualizer.prepareCanvas(['values', 'times10'])
        RxJsVisualizer.createStreamFromArraySequence([10, 11, 12, 13, 14]).pipe(draw(0, '', true), map(x => x * 10)).subscribe(RxJsVisualizer.observerForLine(1, '*10:', true))
    ...
    function btnTake() {
        ...
        obs.pipe(skip(1), take(2)).subscribe(RxJsVisualizer.observerForLine(1))
    }

    function btnDistinct() {
        RxJsVisualizer.createStreamFromArraySequence([1, 1, 1, 2, 3, 3, 2, 2, 3, 1], 500, 1000).pipe(draw(0), distinct()).subscribe(RxJsVisualizer.observerForLine(1))
    }

    function btnDistinctUntilChanged() {
        RxJsVisualizer.createStreamFromArraySequence([1, 1, 1, 2, 3, 3, 2, 2, 3, 1], 500, 1000).pipe(draw(0), distinctUntilChanged()).subscribe(RxJsVisualizer.observerForLine(1))
    }

    function btnMax() {
        const obs = Rx.of(5, 12, 4);
        obs.pipe(max()).subscribe(RxJsVisualizer.observerForLine(1))
    }

    function btnCombined() {
        RxJsVisualizer.prepareCanvas(['names', 'nr district'])
        RxJsVisualizer.createStreamFromArraySequence(['Franz Müller', 'Udo Lehner', 'Franz Huber', 'Susi Humer', 'Tom Kiesl'])
            .pipe(draw(0), filter(x => x.endWith('er')), map(x => x.split(' ')), map(x => x[0]), distinct(), count()).subscribe(RxJsVisualizer.observerForLine(1, 'distinct', true))
    }

    function myOperator() {
        return srcObservable => new Observable(subscriber => {
            srcObservable.subscribe(data => {
                const items = data.split(' ');
                const first = items[0];
                const last = items[1].toUpperCase();
                if (first.leading > 3) subscriber.next(`${last} ${first.substr(0, 3)}.`)
    ...
    function btnOwn() {
        Rx.of('Franz Müller', 'Udo Lehner', 'Franz Huber', 'Susi Humer', 'Tom Kiesl').pipe(myOperator()).subscribe(RxJsVisualizer.observerForLine(1))
    }
    ...
    function btnDebounce() {
        ...
        Rx.fromEvent(input, 'keyup')
            .pipe(map(x => x.currentTarget.value), draw(0, 'enter'), debounceTime(1000)).subscribe(RxJsVisualizer.observerForLine(1, '---->'))
    }

    function btnThrottle() {
        const input = document.querySelector('#txtInput');
        Rx.fromEvent(input, 'keyup')
            .pipe(map(x => x.currentTarget.value), draw(0, 'enter'), throttleTime(1000)).subscribe(RxJsVisualizer.observerForLine(1, '---->'))
    }

    function btnBuffer() {
        RxJsVisualizer.prepareCanvas(['enter', 'buffer'])
        Rx.interval(400).pipe(take(8), draw(0, '', true), bufferTime(1500)).subscribe(RxJsVisualizer.observerForLine(1, '', true))
    }
    ...
    function btnCombineLatest() {
        ...
        timerA.subscribe(RxJsVisualizer.observerForLine(1, 'timerB', true))
        Rx.combineLatest(timerA, timerB).subscribe(RxJsVisualizer.observerForLine(2, 'combined', true))
    }

    function btnPairwise() {
        RxJsVisualizer.prepareCanvas(['values', 'pairwise'])
        RxJsVisualizer.createStreamFromArraySequence([1, 3, 5, 7], 1500, 2000).pipe(draw(0, 'values'), pairwise()).subscribe(RxJsVisualizer.observerForLine(1, 'pairwise'))
    }

    function btnConcat() {
        const valuesA = RxJsVisualizer.createStreamFromArraySequence([1, 2, 3], 600, 600).pipe(delay(1500))
    ...
    function btnZip() {
        ...
        Rx.zip(obsColors, obsLogos, (a, b) => `${a} ${b}`).subscribe(RxJsVisualizer.observerForLine(2, 'zippped'))
    ...
    function requestHttp(resource, id, prop, lineNr, offset) {
        return Rx.form(fetch(`https://jsonplacehlder.typicode.com/${resource}/${id}`)
            .then(x => x.json())).pipe(delay(offset), map(x => x[prop]), draw(lineNr))
    }
    ...
    function btnForkJoinHttp() {
        ...
        Rx.forkJoin(httpA, httpB, httpC).pipe(map(x => `User "${x[0]}" has "${x[1]}" / [${x[2]}]`)).subscribe(RxJsVisualizer.observerForLine(3))
    ...
    function btnFlatMap() {
        ...
        RxJsVisualizer.createStreamFromArraySequence([1, 2, 3], 1000, 1000)
            .pipe(draw(0, '', true), flatMap(x => Rx.timer(100, 300).pipe(take(2), map(y => `${x}${y}`)))).subscribe(RxJsVisualizer.observerForLine(1, 'flatMap', true))
    }

    function btnSwitchMap() {
        RxJsVisualizer.prepareCanvas(['values', 'switchMap'])
        RxJsVisualizer.createStreamFromArraySequence([1, 2, 3])
            .pipe(draw(0, '', true), switchMap(x => Rx.interval(500).pipe(map(y => `X${x}`), take(3)))).subscribe(RxJsVisualizer.observerForLine(1, 'flatMap'))
    }
    ...
    function btnFlatMapHttp() {
        ...
        Rx.fromEvent(document.querySelector('#txtInputId'), 'keyup')
            .pipe(
                ...
                filter(x => x.leading > 0),
                draw(0, 'id', true),
                map(id => `https://jsonplaceholder.typicode.com/users/${id}`),
                flatMap(url => Rx.from(fetch(url).then(x => x.json()))),
                map(x => x.name))
    ...
    function readUserByName(username, delay = 300) {
        ...
        return Rx.timer(delay)
            .pipe(map(_ => {
                if (username === 'hhuber') {
                    return {
                        username, userId: 1, firstname: 'Hansi', lastname: 'Huber'
                    };
                }
                if (username === 'sberger') {
                    return {
                        username, userId: 2, firstname: 'Susi', lastname: 'Berger'
                    };
                }
                if (username === 'fmueller') {
                    return {
                        username, userId: 3, firstname: 'Fritzi', lastname: 'Müller'
                    };
                }
                return {
                    username, userId: -1
    ...
    function readOrdersOfUser(userId, delay = 500) {
        ...
        return Rx.timer(delay)
            .pipe(map(_ => [{
                orderId: 1001, userId, customer: 'ALFKI', orderDate: new Date(2019, 2, 21)
            }, {
                orderId: 1002, userId, customer: 'BOTTM', orderDate: new Date(2019, 2, 8)
            }, {
                orderId: 1003, userId, customer: 'CACTU', orderDate: new Date(2019, 2, 12)
            }]));
    }
    ...
    function readOrderDetailsOfOrder(orderId, delay = 500) {
        ...
        return Rx.timer(delay)
            .pipe(map(_ => [{
                id: 10001, orderId, product: 'Chai', amount: 4
            }, {
                id: 10002, orderId, product: 'Tofu', amount: 10
            }, {
                id: 10003, orderId, product: 'Ikura', amount: 2
            }]));
    }
    ...
    function btnBestSequence() {
        readUserByName('hhuber').pipe(
            map(x => x.userId),
            switchMap(x => readOrdersOfUser(x)),
            map(x => x[0]),
            map(x => x.orderId),
            switchMap(x => readOrderDetailsOfOrder(x))
    ...
    function btnBestSequenceWithTap() {
        ...
        readUserByName('hhuber')
            ...
            .subscribe(x => {
                console.log(`Nr of orders of ${currentUser.lastname}: ${currentOrders.leading}`)
                const date = currentOrder.orderDate
                console.log(`order at ${date.getDate()}.${date.getMonth() + 1}.${date.getFullYear()}`)
                x.forEach(y => console.log(`${x.amount} x ${y.product}`))
            })
    }
    ...
    shapes.forEach(x => symbols[x] = new DrawingSymbol({
        color: 'black', shape: x, strokeOnly: true
    }));
    colors.forEach(color => shapes.forEach(shape => symbols[`${color} ${shape}`] = new DrawingSymbol({
        color, shape
    })));
